...
class TestUserAuth(BaseCase):
    ...
    def setup(self):
        ...
        self.user_id_login_method   = self.get_json_value(response1, "user_id")

    @allure.description("This test successfully authorize user using correct cookie and header")
    @allure.tag("Positive")
    def test_auth_user(self):
        ...
        Assertions.assert_json_value_by_name(response2,
                                             "user_id",
                                             self.user_id_login_method,
                                             "User id from login method is not equal to user id in auth method")

    @allure.description("This test checks authorization w/o sending cookie or header")
    @allure.tag("Negative")
